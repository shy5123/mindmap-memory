...
🐛 修复（同版本补丁）
- 核心记忆恢复：5个被误删的核心节点已找回并重新激活，score 恢复至 ≥3
- 重复节点清理：内容完全相同的副本已去重合并
- 园丁时间戳修复：无嵌入模型时 consolidate_today 也更新 last_consolidate，不再每次 load 空转
- 核心保护测试：pre_release_tests 新增核心记忆保护和删除阻挡两项验证用例
- README / CHANGELOG 同步 v1.5.0 变更

## v1.4.0 (2026-05-01)

**时间戳、JSON 输出、自动安装引导**

- 时间戳记录：每个记忆节点新增 `created_at` 字段（ISO 8601），SQLite 自动加列兼容旧数据库，`recall` 输出含创建时间
...
- `consolidate` CLI 命令：记忆园丁，用嵌入模型重分类当天记忆
...
- 修剪同步：每次写 `MEMORY.md` 前自动扫描原生 `memory` 工具的新增条目并纳入
- 记忆园丁：每次 `load()` 自动检查，距上次超过 24 小时则用嵌入模型重分类当天新增的叶子节点，
  话题树越用越准。也暴露为 `consolidate` CLI 命令。
